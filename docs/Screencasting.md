...
In order to use it, you need a working D-Bus session, pipewire, `xdg-desktop-portal-gnome`, and [running niri as a session](./Getting-Started) (i.e. through `niri-session` or from a display manager).
...
Check [the corresponding wiki section](./Configuration:-Window-Rules#block-out-from) for more details and examples.
...
The [`is-window-cast-target=true` window rule](./Configuration:-Window-Rules#is-window-cast-target) matches windows targeted by an ongoing window screencast.
