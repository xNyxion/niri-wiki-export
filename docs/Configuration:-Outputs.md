...
>
> If the cursor moves at a low framerate with VRR, try setting the [`disable-cursor-plane` debug flag](./Configuration:-Debug-Options#disable-cursor-plane) and reconnecting the monitor.
>
...
By default, hot corners [in the gestures settings](./Configuration:-Gestures#hot-corners) are used for all outputs.
...
It accepts all the same options as [the top-level `layout {}` block](./Configuration:-Layout).
