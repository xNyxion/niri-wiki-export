...
*   with [`focus-follows-mouse`](./Configuration:-Input#focus-follows-mouse), the windows you happen to cross with the mouse pointer on the way to the target window.
...
The recent windows binds have lower precedence than the [normal binds](./Configuration:-Key-Bindings), meaning that if you have <kbd>Alt</kbd><kbd>Tab</kbd> bound to something else in the normal binds, the `recent-windows` bind won't work.
