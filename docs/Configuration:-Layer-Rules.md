...
Please read the [window rules wiki page](./Configuration:-Window-Rules) to learn how matching works.
...
The same caveats and instructions apply as for the [`block-out-from` window rule](./Configuration:-Window-Rules#block-out-from), so check the documentation there.
...
These rules have the same options as the normal [`shadow` config in the layout section](./Configuration:-Layout#shadow), so check the documentation there.
...
Set to `true` to place the surface into the backdrop visible in the [Overview](./Overview) and between workspaces.
...
This is a natural extension of the [April Fools' 2025 feature](./Configuration:-Window-Rules#baba-is-float).
